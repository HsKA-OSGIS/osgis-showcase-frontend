...
//
*/
OSGIS.Search = OpenLayers.Class(OpenLayers.Control, {
    initialize : function(options) {
        ...
        $("#slider-range").slider({
            range: true,
            min: OSGIS.globals['minyear'],
            max: OSGIS.globals['maxyear'],
            values: [OSGIS.globals['minyear'], OSGIS.globals['maxyear']],
            ...
            eventListeners : {
                featureselected: function(evt){
                    ...
                    if (!OSGIS.globals.mobile) {
                        ...
                        if (l.length == 1) {
                            html = $.ajax({
                                url: OSGIS.globals['get_url'] +'/'+ l[0][0],
                                data:{
                                    'format':'html',
                                },
                            dataType: 'html',
                            ...
                            for (i=0;i<l.length;i++){
                                ...
                                html += '<td><a href="'+ OSGIS.globals['apiitem_url'] + l[i][0] + '" target="_blank" class="label">' + l[i][1] + '</a></td>';
                                html += '</tr>';
                            }
                            html += '</table></div>';
                            var popup = new OpenLayers.Popup.FramedDDB(
                        ...
                        if (l.length == 1) {
                            ...
                            for (i=0;i<Math.min(15, l.length);i++){
                                html += '<li><a href="'+ OSGIS.globals['apiitem_url'] + l[i][0] + '" target="_blank" class="label">' + l[i][1] + '</a></li>';
                ...
                featureunselected: function(evt){
                    if(evt.feature.popup) {
                        OSGIS.map.removePopup(evt.feature.popup);
                    }
                }
            }
        })
    ...
    _api_search: function(rc, isEvent){
        ...
        $.get(OSGIS.globals.search_url, params, function(data){
